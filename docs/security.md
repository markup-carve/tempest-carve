...
The registered converter enables `carve-php` safe mode. It escapes raw HTML,
...
Applications that intentionally need trusted raw HTML can inject a separately
configured `CarveConverter` in their own application code. That changes the
security boundary and should be limited to content controlled by trusted
authors. See the
